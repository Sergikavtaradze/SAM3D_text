...
def main():
    ...
    args.snapshot_path = os.path.join(args.snapshot_path, args.data, 
    "MaskWise_Excluded_IDs_" + "lr_" + str(args.lr) + "crop" + str(args.rand_crop_size) + args.date)
    ...
    for i in range(4):
        ...
        prompt_encoder = PromptEncoder_Text()
        ...
        for idx, (img, seg, spacing, mask_type) in enumerate(train_data):
            ...
            for i, (feature, prompt_encoder) in enumerate(zip(feature_list, prompt_encoder_list)):
                if i == 3:
                    ...
                    #print(f"This is the shape of feature: {feature.shape}")
                    prompt_feature = prompt_encoder(feature, text_prompt)
                    #print(f"This is the shape of prompt_feature: {prompt_feature.shape}")
                    new_feature.append(prompt_feature)
                else:
